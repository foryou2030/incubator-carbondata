...
public class CarbonTable implements Serializable {
  ...
  public CarbonTable() {
    ...
    this.tableAllDimensionsMap = new HashMap<String, List<CarbonDimension>>();
  }
  ...
  private void fillDimensionsAndMeasuresForTables(TableSchema tableSchema) {
    ...
    List<CarbonDimension> allDimensions = new ArrayList<CarbonDimension>();
    List<CarbonMeasure> measures = new ArrayList<CarbonMeasure>();
    this.tableDimensionsMap.put(tableSchema.getTableName(), dimensions);
    this.tableAllDimensionsMap.put(this.tableUniqueName, allDimensions);
        ...
        if (columnSchema.getNumberOfChild() > 0) {
          ...
          dimensionOrdinal =
              readAllComplexTypeChildrens(dimensionOrdinal, columnSchema.getNumberOfChild(),
                  listOfColumns, complexDimension, allDimensions);
          ...
          if (!columnSchema.getEncodingList().contains(Encoding.DICTIONARY)) {
            ...
            CarbonDimension dimension =
                    new CarbonDimension(columnSchema, dimensionOrdinal++, keyOrdinal++, -1, -1);
            dimensions.add(dimension);
            allDimensions.add(dimension);
          } else {
            columnGroupOrdinal =
            ...
            CarbonDimension dimension = new CarbonDimension(columnSchema, dimensionOrdinal++,
                    keyOrdinal++, columnGroupOrdinal, -1);
            dimensions.add(dimension);
            allDimensions.add(dimension);
          }
        }
      ...
      if (columnSchema.isDimensionColumn()) {
        if (columnSchema.getNumberOfChild() > 0) {
          ...
          parentDimension.getListOfChildDimensions()
              .add(carbonDimension);
          allDimensions.add(carbonDimension);
        }
      }
    }
  ...
  public CarbonDimension getAllDimensionByName(String tableName, String columnName) {
    List<CarbonDimension> dimList = tableAllDimensionsMap.get(tableName);
    for (CarbonDimension dim : dimList) {
      if (dim.getColName().equalsIgnoreCase(columnName)) {
        return dim;
      }
